...
class _VtsMapsState extends State<VtsMaps> with TickerProviderStateMixin  {
  ...
  final WebSocketChannel channel = WebSocketChannel.connect(Uri.parse(
      'wss://api.binav-avts.id:6001/socket-kapalCoor?appKey=123456&User-Agent=BinavAvts/1.0&Accept=application/json'));
                            ...
                            builder: (BuildContext context) {
                              ...
                              return Dialog(
                                shape: const RoundedRectangleBorder(
                                    borderRadius:
                                        BorderRadius.all(Radius.circular(5))),
                                child: ClientPage(id_client:widget.idClient),
                              );
                            });
                    }
                  },
                ...
                child: StreamBuilder(
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.hasData && snapshot.data != "on Opened") {
                        final data = GetVesselCoor.GetKapalAndCoor.fromJson(
                ...
                Flexible(
                  child: FlutterMap(
                    ...
                    children: [
                      ...
                      PipelineLayer(id_client:widget.idClient),

                      // Vessel Marker (pages/maps/marker_vessel.dart)
                      MarkerVessel(mapController: mapController,id_client:widget.idClient),
                    ],
                  ),
                ),
              ],
